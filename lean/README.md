...
`D >= 2`), any continuous additive map on log-coordinates that is equivariant
under coordinate relabeling, invariant on constant-depth shifts, and calibrated
on the first standard basis vector is exactly `Invariance.clr`.
