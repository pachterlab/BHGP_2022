...
pairwise_pl <- pairwise_df %>%
  ...
  ggplot(aes(x = A, y = B)) +
    ...
    scale_y_grouped_discrete(grouping = ~ trans_families_labels[deframe(trans_families)[.x]],
                                             gap_size = 0.4, limits = rev, labels = trans_labels, expand = expansion(add = 0)) +
    scale_x_grouped_discrete(grouping = ~ fct_rev(trans_families_labels[deframe(trans_families)[.x]]),
