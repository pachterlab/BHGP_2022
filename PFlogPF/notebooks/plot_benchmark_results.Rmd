...
```{r}
library(tidyverse)
# library(gggroupedscale)  # incompatible with ggplot2 >= 4.0
# Use ggh4x's nested axis guide as the replacement — provides the bracketed
# family labels (Delta method / GLM residuals / Lat. Expr. / Count / Neg.)
# on the y-axis that match AE&H 2023 (Nature Methods) Figure 2.
library(ggh4x)
scale_y_grouped_discrete <- function(..., grouping = NULL, gap_size = 1, limits = NULL,
  ...
  if (add_group_label && !is.null(grouping) && !is.null(labels)) {
    # Prepend family label using a delimiter so ggh4x::guide_axis_nested
    # can split into a 2-level nested guide.
    methods <- names(labels)
    grouping_fn <- if (inherits(grouping, "formula")) rlang::as_function(grouping)
                   else if (is.function(grouping))    grouping
                   else                               function(.x) grouping[.x]
...
scale_x_grouped_discrete <- function(..., grouping = NULL, gap_size = 1, limits = NULL,
                                    add_group_label = FALSE, guide = NULL) {
  args <- list(...)
  if (!is.null(limits)) args[["limits"]] <- limits
  if (!is.null(guide)) args[["guide"]] <- guide_axis(angle = 90)
  do.call(scale_x_discrete, args)
}
theme_grouped_axis <- function(...) theme()
guide_grouped_axis <- function(...) guide_axis(...)
# Use ragg backend for ggrastr (no X11/Cairo required)
...
make_main_plot_panel <- function(data, add_group_label = FALSE){
  ggplot(data, aes(x= knn_recovery, y = transformation, color = family)) +
    ...
    ggbeeswarm::geom_quasirandom(color = "grey", size = 0.4, alpha = 0.7, groupOnX = FALSE) +
    # Family-colored mean dot, prominent (matches AE&H Fig 2 styling).
...
simulation_pl <- res_main %>%
  ...
  # Same widening rationale as the downsampling panel: PFlogPF (shift. CLR) lands
  # above the default xlim because of its scale-invariance, so its mean dot
  # would be clipped off-screen. Widen to match downsampling's range.
...
downsampling_pl <- res_main %>%
  ...
  make_main_plot_panel(add_group_label = FALSE) +
  # PFlogPF (shift. CLR) is ~5x more depth-robust than every other method, so its
  # normalized recovery sits around 5; widen the downsampling-panel xlim so
  # those datapoints are visible. Other 2 panels keep the default [0.2, 1.8].
...
```{r, error=TRUE}
# Panel D (k-NN overlap depends on #PCA-dimensions) is removed to match the
# requested 3-panel layout. Lowercase panel labels (a/b/c) and reduced height
# also match AE&H Fig 2 style.
plot_assemble(annotate_text("a", x = 2,   y = 1, vjust = 1, fontsize = font_size, fontface = "bold"),
              ...
              # annotate_graphic y is FROM-BOTTOM (per utils.R comment);
              # 91mm from bottom of 102mm canvas places schematics near top
              annotate_graphic("../illustrations/benchmark_overviewArtboard_1.pdf", x = 32,  y = 91, units = "mm"),
              annotate_graphic("../illustrations/benchmark_overviewArtboard_3.pdf", x = 92,  y = 91, units = "mm"),
              annotate_graphic("../illustrations/benchmark_overviewArtboard_2.pdf", x = 152, y = 91, units = "mm"),
              # plots use plotgardener (y from top); start below schematics
...
Stand-alone version of just the siRNA-KD k-NN-vs-PCA-dimensions panel
(originally the last sub-panel of AE&H Fig 2 panel d). PFlogPF (shift. CLR)
appears as a red line because the compositional family color flows through
`pca_dat`. Uses ggsave (no tikz) so it renders independently of the main
figure's tikz/pdflatex post-processing.

```{r, error=TRUE}
# Rebuild from pca_dat instead of decorating the small original (which
# carries breaks = c(0, 5) inherited from the AE&H mini-panel sizing).
sirna_pca_standalone <- pca_dat %>%
  filter(dataset == "smartSeq3_siRNA_knockdown") %>%
  ggplot(aes(x = pca_dim, y = knn_recovery)) +
  geom_line(aes(group = transformation, color = family, linetype = is_dim_indep),
  ...
  scale_color_manual(values = trans_families_colors) +
  scale_linetype_manual(values = c(`TRUE` = "dashed", `FALSE` = "solid")) +
  labs(subtitle = "siRNA KD (Downsampling)",
       x = "Number of PCA dimensions",
       y = "k-NN overlap") +
...
ggsave("../output/sirna_pca_dependence.pdf", sirna_pca_standalone,
       width = 100, height = 70, units = "mm", device = cairo_pdf)
ggsave("../output/sirna_pca_dependence.png", sirna_pca_standalone,
...
scaling_plot <- res %>%
  ...
  ggplot(aes(x = n_cells, y = cputime_sec)) +
    ggrastr::rasterise(ggbeeswarm::geom_quasirandom(alpha = 0.2, color = "grey", size = 0.1), dpi = 300, dev = "ragg_png") +
