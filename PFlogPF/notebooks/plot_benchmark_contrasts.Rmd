...
scale_y_grouped_discrete <- function(..., grouping = NULL, gap_size = 1, limits = NULL) {
  args <- list(...)
  args[["limits"]] <- limits
  do.call(scale_y_discrete, args[!sapply(args, is.null)])
}
scale_x_grouped_discrete <- function(..., grouping = NULL, gap_size = 1, limits = NULL) {
  args <- list(...)
  args[["limits"]] <- limits
  do.call(scale_x_discrete, args[!sapply(args, is.null)])
}
...
alpha_comps_pl <- cowplot::plot_grid(plotlist = lapply(alpha_pl_tmp, function(x) if(is.null(x)) NULL else x + alpha_theme_adapt),
...
loess_fits <- full_knn_overlaps %>%
  ...
  reframe(broom::augment(loess(overlap ~ log_seq_depth, span = 0.3))) %>%
...
knn_per_depth_pl <- full_knn_overlaps %>%
  ...
  ggplot(aes(x = seq_depth, y = overlap)) +
    ...
    ggrastr::rasterise(geom_point(alpha = 0.01, size = 0.01), dpi = 300, dev = "ragg_png") +
