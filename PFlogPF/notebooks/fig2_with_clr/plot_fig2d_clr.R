...
# plot_fig2d_clr.R
#
# Reproduces Figure 2d (siRNA KD downsampling panel) with PFlogPF (shift. CLR) added.
...
# Rename CLR to PFlogPF (shift. CLR)
...
label_pts <- sirna_dat %>%
  ...
  ungroup() %>%
  mutate(label = c(clr = "PFlogPF (shift. CLR)", scgpt = "scGPT")[as.character(transformation)])
