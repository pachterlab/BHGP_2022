...
sirna_quantitative_recovery <- readRDS(file.path("output/job_storage/sirna_quantitative_recovery_job.RDS"))
...
if(job_status(sirna_quantitative_recovery) == "done"){
  message("save sirna_quantitative_recovery")
  file.copy(file.path(.OUTPUT_FOLDER, "results", sirna_quantitative_recovery$result_id), "output/benchmark_results/sirna_quantitative_recovery_results.tsv", overwrite = TRUE)
}else{
  benchmark_complete <- FALSE
  job_stats <- sapply(sirna_quantitative_recovery$dependencies, job_status)
  message("Job stats for sirna_quantitative_recovery")
  print(table(job_stats))
}
