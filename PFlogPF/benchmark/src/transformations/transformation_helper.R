...
make_lowdim_representation <- function(transformation, dat, pca_dim){
  if(transformation == "sanity_dists"){
    dist_obj <- if(inherits(dat, "dist")) dat else as.dist(dat)
    k <- min(pca_dim, attr(dist_obj, "Size") - 1L)
    red_dat <- cmdscale(dist_obj, k = k)
    if(is.null(dim(red_dat))){
      red_dat <- matrix(red_dat, ncol = 1L)
    }
    as.matrix(red_dat)
  }else if(transformation == "glmpca"){
    pca_dim <- min(pca_dim, nrow(dat), ncol(dat))
    pca_res <- glmpca::glmpca(
      Y = dat, L = pca_dim,
      fam = if(isFALSE(attr(dat, "alpha"))) "poi" else "nb",
      sz = attr(dat, "size_factor"), nb_theta = 1 / attr(dat, "alpha")
    )
    as.matrix(pca_res$factors)
  }else if(transformation == "newwave"){
    pca_dim <- min(pca_dim, nrow(dat) - 1L, ncol(dat) - 1L)
    se <- SummarizedExperiment::SummarizedExperiment(assay = list(counts = dat))
    pca_res <- NewWave::newWave(Y = se, K = pca_dim, n_gene_disp = 100, children = 1)
    as.matrix(SingleCellExperiment::reducedDim(pca_res, "newWave"))
  }else{
    if(pca_dim >= nrow(dat) || pca_dim >= ncol(dat)){
      red_dat <- t(as.matrix(dat))
    }else if(pca_dim >= nrow(dat) / 2 || pca_dim >= ncol(dat) / 2){
      red_dat <- BiocSingular::runPCA(
        t(dat), rank = pca_dim, get.rotation = FALSE,
...
# scGPT-style value binning: for each cell, bin non-zero counts into B
# within-cell quantile bins. Zeros stay zero.
scgpt_bin_nonzero <- function(x, n_bins = 51L){
  x <- as.numeric(x)
  if(length(x) == 0){
    return(integer())
  }
  ranks <- rank(x, ties.method = "max")
  pmax.int(1L, pmin.int(n_bins, as.integer(ceiling(ranks * n_bins / length(x)))))
}

scgpt_fnc <- function(UMI, sf, alpha, n_bins = 51L){
  if(inherits(UMI, "dgCMatrix")){
    res <- methods::as(UMI, "dgCMatrix")
    for(col_idx in seq_len(ncol(res))){
      start <- res@p[col_idx] + 1L
      end   <- res@p[col_idx + 1L]
      if(start <= end){
        res@x[start:end] <- scgpt_bin_nonzero(res@x[start:end], n_bins = n_bins)
      }
    }
    return(res)
  }
  ...
  if(inherits(UMI, "sparseMatrix")){
    return(scgpt_fnc(methods::as(UMI, "dgCMatrix"), sf, alpha, n_bins = n_bins))
  }

  res <- as.matrix(UMI)
  for(col_idx in seq_len(ncol(res))){
    nz_idx <- which(res[, col_idx] > 0)
    if(length(nz_idx) > 0){
      res[nz_idx, col_idx] <- scgpt_bin_nonzero(res[nz_idx, col_idx], n_bins = n_bins)
    }
  }
  res
}
