...
def main(dsdir, celltype):
    ...
    with gzip.open(f"{dsdir}/subset_genes/clr.csv.gz", "rt") as f:
        add = np.loadtxt(f, delimiter=",", dtype=np.float32)
    ...
    print(f"raw {raw.shape} | mult {mult.shape} | add {add.shape} | {celltype}: {idx.size} cells\n")

    for name, X in [("multiplicative pf_log_pf", mult), ("additive CLR (norm_clr)", add)]:
        print(f"=== {name} ===")
