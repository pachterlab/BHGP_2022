...
def plot_data(axs, data):
    ...
    for (ax1, ax2, ax3), title in zip(axs, labels):
        ...
        try:
            ...
            ax2.text(0.5, 0.5, 'Computationally intractable',
...
#     base_out_fn =  os.path.join("synchromesh/data/", ds)
    ...
    metrics = plot_data(axs, data)
    
    fig.savefig(os.path.join(base_out_fn, f"{ds}_full_method_comparison.pdf"), facecolor='white', transparent=False, dpi=300, bbox_inches="tight")
    return
