...
def metrics_gcov(mtx):
    ...
    mtx_squared.data **= 2
    gvar =  np.asarray(mtx_squared.mean(0) - np.square(mtx.mean(0))).ravel()
    
    gcov = np.sqrt(np.var(gvar))/np.mean(gvar)
    return gcov
    
def metrics_ccov(mtx):
    csum = np.asarray(mtx.sum(1)).ravel()
    
    ccov = np.sqrt(np.var(csum))/np.mean(csum)
    return ccov

def metrics_depth(mtx, raw, same=False):
    x = np.asarray(raw.sum(1)).ravel()
    y = np.asarray(mtx.sum(1)).ravel() 
